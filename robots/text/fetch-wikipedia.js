...
const WikipediaFactory = function (fetchMethod) {
  if (this instanceof WikipediaFactory) {
    const s = new this[fetchMethod]()
    return s
  } else {
...
WikipediaFactory.prototype = {
  ...
  WikipediaAPI: function () {
    this.fetch = async function (content) {
      ...
      const RealText = await getRealText(content.searchTerm, content.lang)
      title = RealText
      console.log('Searching content...')
      await getContent(content.lang)
       ...
       */
      async function getRealText (text, lang = 'en') {
        ...
        if (res.body[1].length === 0) {
          ...
          console.log(
            'Tip: Search your therm in English or pre-search valid Words'
          )
        ...
        const index = await selectTerm(sugestions)
        if (index === -1) {
       ...
       *
       */
      async function getContent (lang = 'en') {
        ...
        let value
        const map = new Map(Object.entries(ret.body.query.pages))
        ...
        for (let i = 0; i < value.images.length; i++) {
          await getURLImage(value.images[i].title, lang)
        }
      }
      /*
       *
